...
class GlobalCD {
	constructor() {
		...
		this.cooldown['vip'] = this.cooldown[roles['vip'].id] = new Objectify(10000); //10 seconds
		this.cooldown['resident-dj'] = this.cooldown[roles['resident-dj'].id] = new Objectify(20000); //20 seconds
		this.cooldown['user'] = this.cooldown[roles['user'].id] = new Objectify(40000); //40 seconds
	...
	used(user) {
		if (this.cooldown[this.user.roleid] > 0) {
			...
			}, this.cooldown[this.user.roleid].value);
		}
	}
}
